...
"vim" : "Vim",
"SGIv1" : "SGI-1",
"PD" : "Software released to the public domain",
}
    ...
    try:
        ...
        with os.fdopen(fh, 'w') as new_file:
            with open(fn, "r") as old_file:
                my_print("Open file: %s" % fn)
                for line in old_file:
                    #if not line.startswith("licenseDeclared"):
                    if "licenseDeclared" not in line:
                        new_file.write(line)
                        continue
                    my_print("Found pattern: licenseDeclared")
                    # license_map
                    orig = line
                    ...
                    # license_concat_map
                    for license in sorted(license_concat_map, key=len, reverse=True):
                        for ending in ['"', "'", " ", ")"]:
                            line = line.replace(license + ending, license_concat_map[license] + ending)                    
                    
                    if orig != line:
                        my_print("Replace license name " +orig.strip()+" with "+line.strip())
                        modified = True
                    new_file.write(line)
        new_file.close()
        if modified:
            ...
            my_print(" *** END process file: %s ***" % fn)
...
for targetdir in sys.argv[1:]:
    my_print("processing directory '%s'" % targetdir)
